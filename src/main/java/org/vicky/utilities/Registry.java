 ...
 * @param <R> the type of the registry subclass
 */
public abstract class Registry<T, R extends Registry<T, R>> {
  ...
  private void registerInstance() {
    ...
    if (instances.containsKey(clazz)) {
      logger.printBukkit(
          "Attempt to register multiple instances of " + clazz.getSimpleName(),
   ...
   * @param <R>   The registry class itself
  ...
  public static <T, R extends Registry<T, R>> R getInstance(Class<R> clazz) {
    ...
    if (instance == null) {
      throw new IllegalStateException(
          "Registry " + clazz.getSimpleName() + " accessed before initialization!");
    }
    return (R) instance;
  }
