...
public class ExtendedItemAnimationController<T extends ExtendedDescriptorItem> extends AnimationController<T> {
    ...
    protected PlayState handleAnimationState(AnimationState<T> state) {
        ...
        int handValue = getHandValue(slot, entity);
        MolangParser.INSTANCE.register(new Variable("core.hand", handValue));
        ...
        if (this.triggeredAnimation != null) {
            ...
            if (!hasAnimationFinished() && (!this.handlingTriggeredAnimations ||
                    this.stateHandler.handle(
                            new ExtendedItemAnimationState<>(this, state)) == PlayState.CONTINUE))
        ...
        return this.stateHandler.handle(new ExtendedItemAnimationState<>(this, state));
    }

    static int getHandValue(EquipmentSlot slot, Entity entity) {
        int handValue = 1; // Default to right (1)
        if (slot != null) {
            boolean isMainHand = (slot == EquipmentSlot.MAINHAND);
            if (entity instanceof LivingEntity living) {
                // If player is right-handed: MAINHAND = Right (1), OFFHAND = Left (0)
                // If player is left-handed:  MAINHAND = Left (0), OFFHAND = Right (1)
                boolean isRightHand = (living.getMainArm() == HumanoidArm.RIGHT) == isMainHand;
                handValue = isRightHand ? 1 : 0;
            } else {
                // Fallback for non-living entities (e.g., armor stands)
                handValue = isMainHand ? 1 : 0;
            }
        }
        return handValue;
    }
