...
import org.vicky.bukkitplatform.*;
...
public final class vicky_utils extends JavaPlugin implements PlatformPlugin {
	...
	private PlatformRankService rankService;
	...
	public static void hookDependantPlugin(@NotNull JavaPlugin plugin, @NotNull ClassLoader loader) {
		Bukkit.getLogger().info(ANSIColor.colorize("New plugin hooked successfully: green[" + plugin.getName() + "]"));
	...
	public static void unhookDependantPlugin(@NotNull JavaPlugin plugin) {
		...
		if (removed) {
			...
			getPlugin().getLogger()
					.warning(ANSIColor.colorize("Plugin bold[" + plugin.getName()
	...
	public static String getHookedDependantPlugins() {
		...
		String plugins = hookedPlugins.stream().map(plugin -> {
	...
	public static void registerTemplatePackage(String jarName, String packageName) {
		...
		new ContextLogger(ContextLogger.ContextType.FEATURE, "HIBERNATE-TEMPLATE")
				.print("Added template package " + ANSIColor.colorize("yellow[" + packageName + "]"));
	}
	...
	public static void registerStatusEffect(List<DeathCause> deathMessages, StatusEffect StatusEffect) {
		pendingEffects.put(StatusEffect, deathMessages);
		new ContextLogger(ContextLogger.ContextType.SYSTEM, "EFFECTS")
				.print("Added custom effect" + ANSIColor.colorize("yellow[" + StatusEffect.getKey() + "]"));
	}

	/**
	 * Called by dependent plugins to register their mapping class utilities like
	 * Enums, Classes and Objects.
	 */
	@Deprecated(forRemoval = true)
	public static void registerTemplateUtilityPackage(String jarName, String packageName) {
		...
		new ContextLogger(ContextLogger.ContextType.FEATURE, "HIBERNATE-TEMPLATE-UTIL")
				.print("Added template utility package " + ANSIColor.colorize("yellow[" + packageName + "]"));
	}
	...
	public void onLoad() {
		try {
			...
			CommunicatorServer.getInstance().register("Vicky-s_Utilities", loader, new VickyUtilsCommunicator());
		} catch (Exception e) {
	...
	public void onEnable() {
		if (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null
			...
			Bukkit.getScheduler().runTaskTimer(this, org.vicky.musicPlayer.MusicPlayer.INSTANCE::tickAll, 1L, 1L);
			...
			try {
				...
				for (var effect : effects.entrySet()) {
					EffectRegistry.getInstance(EffectRegistry.class).registerMechanicEffect(effect.getKey(), null);
				...
				getLogger().info(ANSIColor.colorize("Effects and DeathMessages have been successfully Registered.",
						ANSIColor.PURPLE));
			} catch (Exception e) {
				exceptionOccurred = true;
				getLogger().info(
						ANSIColor.colorize("Unable to register Effects and DeathMessages..... Error:" + e.getMessage(),
								ANSIColor.RED_BOLD));
			} finally {
				if (exceptionOccurred) {
					getLogger().info(
							ANSIColor.colorize("Continuing with Loading Some Errors Might Occur", ANSIColor.LIGHT_RED));
				}
			}
			...
			getServer().getScheduler().scheduleSyncDelayedTask(this, () -> {
				getLogger().info(ANSIColor.colorize("Placeholders from Plugin and its Addons have been registered: ",
						ANSIColor.CYAN));
			...
			exceptionOccurred = true;
			getLogger().severe(
					ANSIColor.colorize("Could not find PlaceholderAPI! This plugin is required.", ANSIColor.RED));
			Bukkit.getPluginManager().disablePlugin(this);
		} else {
			exceptionOccurred = true;
			getLogger().severe(
					ANSIColor.colorize("Could not find MythicAPI! Ensure MythicMobs is Present.", ANSIColor.RED));
			Bukkit.getPluginManager().disablePlugin(this);
		}
	}
	...
	private void registerCommands() {
		new CommandAPICommand("afk").withAliases("goakf", "ibegone", "cya").executesPlayer(executionInfo -> {
			afkManager.handleAfkCommand(executionInfo.sender());
		}).register(this);

		new CommandAPICommand("play").withAliases("play_piece")
				.withArguments(new StringArgument("music_piece")
						.replaceSuggestions(ArgumentSuggestions.strings(info -> MusicRegistry
								.getInstance(MusicRegistry.class).getRegisteredEntities().stream().map(MusicPiece::key)
								.filter(k -> k.startsWith(info.currentArg().toLowerCase())).toArray(String[]::new))))
				.executesPlayer(info -> {
					String musicPiece = info.args().get("music_piece").toString();
					MusicRegistry.getInstance(MusicRegistry.class).playPiece(musicPiece,
							BukkitPlatformPlayer.of(info.sender()));
				}).register(this);

		new CommandAPICommand("toggleMusic").executesPlayer(info -> {
			MusicRegistry.getInstance(MusicRegistry.class).getPlayer()
					.togglePause(BukkitPlatformPlayer.of(info.sender()));
		}).register(this);

		new CommandAPICommand("music_piece")
				.withArguments(List.of(new StringArgument("action")
						.replaceSuggestions(ArgumentSuggestions.strings("page")).setOptional(true)))
				.executesPlayer(info -> {
					String action = info.args().get("action") != null ? info.args().get("action").toString() : "";
					if (action.equals("page")) {
						info.sender().sendMessage(MusicRegistry.getInstance(MusicRegistry.class)
								.renderMusicPage(BukkitPlatformPlayer.of(info.sender()), 0));
					}
				}).register(this);

		new CommandAPICommand("mortalise").withPermission(CommandPermission.OP).executesPlayer(p -> {
			mortalise(p.sender());
		}).register(this);

		new CommandAPICommand("veffect").withAliases("status_effect", "se").withArguments(List.of(
				new EntitySelectorArgument.OneEntity("entity"),
				new StringArgument("context_effect").replaceSuggestions(
						ArgumentSuggestions.strings(info -> EffectRegistry.getInstance(EffectRegistry.class)
								.getRegisteredEntities().stream().map(StatusEffect::getKey)
								.filter(k -> k.startsWith(info.currentArg().toLowerCase())).toArray(String[]::new))),
				new IntegerArgument("duration_in_seconds").setOptional(true),
				new IntegerArgument("level").setOptional(true))).executes(info -> {
					...
					if (level > 225) {
						info.sender().sendMessage(BukkitHex.colorize("red[Your level cannot be more than 225...]"));
						return;
					}
					EffectRegistry.getInstance(EffectRegistry.class).apply(effectName,
							(LivingEntity) info.args().get("entity"), duration, level);
				}).register(this);
	}

	private void getInBuiltEffects() {
		pendingEffects.put(new Bleeding(plugin), List.of(
				new DeathCause("vicky_utils_bleeding", "{player} succumbed to their wounds.", false,
						"still bleeding badly", 1),
				new DeathCause("vicky_utils_bleeding", "{player} bled out.", false, "leaving a trail behind", 1),
				new DeathCause("vicky_utils_bleeding", "{player} bled to death while trying to outrun {killer}", true,
						"gushing blood as they fled from {killer}", 1),
				new DeathCause("vicky_utils_bleeding", "{player} collapsed in a pool of their own blood.", false,
						"weakened by blood loss", 1)));
	}

	private void prepareGlobals() {
		globalConfigManager = new ConfigManager("global/global_configs.yml");
	...
	public void onDisable() {
		if (exceptionOccurred) {
			getLogger().info(ANSIColor.colorize("Rendering Utilities Inaccessible. Dependant Plugins will not Work",
					ANSIColor.RED_BOLD));
		} else {
	...
	private void prepareConfiguration() {
		...
		config.addConfig("defaults.AFKKickMessage",
				new AllowedString("You have been kicked for being AFK too long...This is for your own safety and to"
						+ " save server resources :D"));
		config.addConfig("defaults.BTKMessages",
				new AllowedList<>(List.of("You are no longer AFK... now do something", "Welcome back...")));
		config.addConfig("defaults.AFKMessages",
	...
	private void doDefaults() {
		...
		sqlManager = new SQLManagerBuilder().addMappingClass(DatabasePlayer.class).addMappingClass(Theme.class)
	...
	public void extractFolderFromJar(String folderPath, File destinationFolder) {
		...
		try {
			getLogger().info(ANSIColor.colorize("orange[Extracting folder:] yellow[" + folderPath
					+ "] orange[to] yellow[" + destinationFolder + "]"));
			...
			while (entries.hasMoreElements()) {
				...
				if (entry.getName().startsWith(folderPath + "/") && !entry.isDirectory()) {
					...
					File destFile = new File(destinationFolder, entry.getName().substring(folderPath.length() + 1)); // Adjust
																														// index
																														// for
																														// the
																														// slash
	...
	public PlatformParticleProvider getParticleProvider() {
		return new BukkitParticleProvider();
	}

	@Override
	public PlatformChatFormatter getChatFormatter() {
		return new BukkitChatFormatter();
	}

	@Override
	public PlatformConfig getPlatformConfig() {
		return new BukkitPlatformConfig();
	}

	@Override
	public PlatformBossBarFactory getPlatformBossBarFactory() {
		return new BukkitBossBarFactory();
	}
	...
	public Optional<PlatformPlayer> getPlatformPlayer(UUID uuid) {
		return Optional.ofNullable(BukkitPlatformPlayer.of(Bukkit.getPlayer(uuid)));
	}

	@Override
	public PlatformLogger getPlatformLogger() {
		return new BukkitPlatformLogger();
	}
}
