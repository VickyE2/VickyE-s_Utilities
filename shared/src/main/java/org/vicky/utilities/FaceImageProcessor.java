...
public class FaceImageProcessor {
	...
	private final File facesFolder = PlatformPlugin.dataFolder();
	 ...
	 * @param uuid
	 *            The player's UUID, used for saving the image file.
	 * @param width
	 ...
	 */
	public void processAndSavePlayerFace(String skinURL, UUID uuid, int width, int height) throws IOException {
		// Check if the .puuid image already exists
		File faceFile = new File(facesFolder, uuid.toString() + ".puuid");
		if (faceFile.exists()) {
			PlatformPlugin.logger().info(
					ANSIColor.colorize("Face image already exists as .puuid for UUID: " + uuid, ANSIColor.YELLOW));
			return;
		}
		...
		PlatformPlugin.logger().info(ANSIColor.colorize("Face image saved for UUID: " + uuid, ANSIColor.GREEN));
	}
	 ...
	 *            The player's UUID.
	 * @throws IOException
	 *             if any error occurs during the conversion.
	 */
	public void convertPuuidToPng(UUID uuid) throws IOException {
		...
		if (!puuidFile.exists()) {
			PlatformPlugin.logger()
					.warn(ANSIColor.colorize("No .puuid file found for UUID: " + uuid, ANSIColor.RED_BOLD));
			PlatformPlugin.logger().info(ANSIColor.colorize("creating puuid file for UUID: " + uuid, ANSIColor.GREEN));
			...
			try {
				...
				PlatformPlugin.logger().warn("Error generated during wait: " + e);
				e.printStackTrace();
			}
