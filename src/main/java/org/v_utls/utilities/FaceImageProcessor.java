...
public class FaceImageProcessor {
    ...
    public void processAndSavePlayerFace(String skinURL, UUID uuid, int width, int height) throws IOException {
        ...
        if (faceFile.exists()) {
            vicky_utils.plugin.getLogger().info(ANSIColor.colorize("Face image already exists as .puuid for UUID: " + uuid.toString(), ANSIColor.YELLOW));
            return;
        }
        ...
        vicky_utils.plugin.getLogger().info(ANSIColor.colorize("Face image saved for UUID: " + uuid.toString(), ANSIColor.GREEN));
    }
    ...
    public void convertPuuidToPng(UUID uuid) throws IOException {
        ...
        if (!puuidFile.exists()) {
            vicky_utils.plugin.getLogger().warning(ANSIColor.colorize("No .puuid file found for UUID: " + uuid.toString(), ANSIColor.RED_BOLD));
            vicky_utils.plugin.getLogger().info(ANSIColor.colorize("creating puuid file for UUID: " + uuid.toString(), ANSIColor.GREEN));
