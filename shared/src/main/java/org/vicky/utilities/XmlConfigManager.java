...
public class XmlConfigManager {
	 ...
	 *
	 * @param configFile
	 *            The path of the config file (.xml)
	 */
	public XmlConfigManager(File configFile) {
		this.configFile = configFile;
		this.scheduler = PlatformPlugin.server().getScheduler();
	}
	...
	/**
	 * Creates and initializes a configuration file located in the plugin's data
	 * folder. Automatically sets up a file watcher to reload on changes.
	 *
	 * @param path
	 *            The relative path to the configuration file within the plugin's
	 *            data folder.
	 */
	...
	/**
	 * Creates and initializes a configuration file at an absolute path on the file
	 * system. Automatically sets up a file watcher to reload on changes.
	 *
	 * @param path
	...
	/**
	 * Initializes the configuration using a pre-assigned {@code configFile}. This
	 * method will throw an exception if the config file has not been set.
	 * Automatically sets up a file watcher to reload on changes.
	 *
	 * @throws IllegalStateException
	 *             If {@code configFile} has not been assigned.
	 */
	 ...
	 * @param defaultKey
	 *            The default root XML tag to use if the file is created.
	 */
	...
	/**
	 * Creates and initializes a configuration file at an absolute path with a
	 * custom default root tag.
	 * <p>
	 * This version does not enable automatic file watching.
	 *
	 * @param path
	...
	private void setupConfig(File file, @Nullable String defaultKey, boolean watch) {
		...
		var builder = XmlConfigurationLoader.builder().headerMode(HeaderMode.PRESET).indent(4).path(file.toPath())
				.sink(AtomicFiles.atomicWriterFactory(file.toPath(), UTF_8)).writesExplicitType(false);
