...
class VickyUtilsCommunicator extends Communicateable {
	...
	@Handler(key = "test", description = "Just tests if the communicator works. pass a plugin name \"plugin\"")
	public CompletableFuture<Void> handleTest(JsonObject payload) {
		getLogger().print(
				"Received test payload. If you see this it works... gotten from [" + payload.get("plugin") + "] Plugin",
				ContextLogger.LogType.SUCCESS);
		return CompletableFuture.completedFuture(null);
	}

	@Handler(key = "openGui", description = """
	...
	public CompletableFuture<JsonObject> handleOpenGui(JsonObject payload) {
		...
		JavaPlugin plugin = (JavaPlugin) Bukkit.getPluginManager().getPlugin(payload.get("plugin").getAsString());
		Player player = Bukkit.getPlayer(playerId);

		if (player == null || !player.isOnline()) {
			getLogger().print("Player not found for openGui!", ContextLogger.LogType.WARNING);
			return CompletableFuture.failedFuture(new IllegalArgumentException("Player not online"));
		}
		if (plugin == null) {
			getLogger().print("Plugin not found for openGui!", ContextLogger.LogType.WARNING);
			return CompletableFuture.failedFuture(new IllegalArgumentException("Plugin not found"));
		}

		String title = payload.has("title") ? payload.get("title").getAsString() : "Ecosystem Opened GUI";
		...
		for (JsonElement element : itemsJson) {
			...
			GuiCreator.ItemConfig item = GuiCreator.ItemConfig.fromJson(itemObj, plugin); // <-- You need this method
			items.add(item);
		}
		...
		switch (guiType.toLowerCase()) {
			...
			case "paged", "paginated", "page" -> {
				...
				int itemsPerPage = payload.has("itemsPerPage") ? payload.get("itemsPerPage").getAsInt() : 28;
				boolean persistentButtons = payload.has("persistent") && payload.get("persistent").getAsBoolean();
                ...
                creator.paginated(player, height, GuiCreator.ArrowGap.valueOf(
						...
						items, // pageItems
						range, page, itemsPerPage, title, persistentButtons, textured, textureKey, offset);
			}
			case "anvil", "input" -> {
				if (items.size() < 3) {
					getLogger().print("Anvil GUI requires 3 item configs (left, right, output)",
							ContextLogger.LogType.WARNING);
					return CompletableFuture
							.failedFuture(new IllegalArgumentException("Not enough items for anvil GUI"));
				}

				String initialText = payload.has("initialText") ? payload.get("initialText").getAsString() : "";
                ...
                creator.anvil(player, initialText, textured, textureKey, offset, items.get(0), // left
						...
						title, canClickLeft, canClickRight, (p, state, slot) -> List.of(), // Placeholder completion
																							// logic
						(state) -> {
						} // Placeholder onClose logic
				);
			}
			default -> {
				getLogger().print("Unknown GUI type: " + guiType, ContextLogger.LogType.WARNING);
	...
	public CompletableFuture<Boolean> handleEffectRegister(JsonObject payload) {
		...
		if (optional.isPresent()) {
			...
			getLogger().print("Unknown player with uuid: " + playerUUID, true);
		}
		getLogger().print("Unknown effect with id: " + effectId, true);
		return CompletableFuture.completedFuture(true);
	}
}
