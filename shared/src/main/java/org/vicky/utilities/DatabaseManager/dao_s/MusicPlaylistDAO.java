...
public class MusicPlaylistDAO {
  ...
  public MusicPlaylist findById(String id) {
    try (EntityManager em = HibernateUtil.getEntityManager()) { // Open new EntityManager
      MusicPlaylist piece =
          em.createQuery("FROM MusicPlaylist WHERE playlistId = :id", MusicPlaylist.class)
              .setParameter("id", id)
              .getSingleResult();
  ...
  public List<MusicPlaylist> getAll() {
    try (EntityManager em = HibernateUtil.getEntityManager()) { // Open new EntityManager
      List<MusicPlaylist> pieces =
          em.createQuery("SELECT t FROM MusicPlaylist t", MusicPlaylist.class).getResultList();
      em.close();
      return pieces;
    } catch (NoResultException e) {
      return null;
   ...
   *
   * @param theme the MusicPlaylist to persist
   */
   ...
   *
   * @param theme the MusicPlaylist to update
