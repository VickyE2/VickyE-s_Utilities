		...
		if (oldState != null && newState != null) {
			for (int i = 0; i < propertyNames.length; i++) {
				...
				if (oldVal == null || !oldVal.equals(newVal)) {
					changes.append(
							String.format("Field '%s' ~ Altered '%s' to '%s'. ", propertyNames[i], oldVal, newVal));
				}
			}
		}
	...
	private void logChange(String operation, Object entity, String details) {
		...
		try {
			...
			if (PlatformPlugin.config().getBooleanValue("Debug"))
				logger.print(
						"Unable to retrieve ID for entity: " + entityName
								+ ". It probably extends a class that has the parameter...",
						ContextLogger.LogType.WARNING);
		}

		String format = String.format("%s ~ %s having Id -> %s%s", operation, entityName, entityId,
