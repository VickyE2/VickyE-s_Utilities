...
public class HexGenerator {
	...
	// Interpolates between two colors based on the ratio (0.0 = startColor, 1.0 =
	// endColor)
	...
	/**
	 * Converts ANSI escape sequences for RGB colors (e.g. "\u001B[38;2;R;G;Bm") to
	 * the Bukkit hex format ("§x§R§R§G§G§B§B").
	 *
	 * @param message
	 *            The message containing ANSI escape sequences.
	 * @return The message with ANSI escape sequences replaced by Bukkit hex color
	 *         codes.
	 */
	public static String convertAnsiEscapeToBukkitHex(String message) {
		...
		while (matcher.find()) {
			...
			String bukkitHex = COLOR_CHAR + "x" + COLOR_CHAR + hex.charAt(1) + COLOR_CHAR + hex.charAt(2) + COLOR_CHAR
					+ hex.charAt(3) + COLOR_CHAR + hex.charAt(4) + COLOR_CHAR + hex.charAt(5) + COLOR_CHAR
					+ hex.charAt(6);
	...
	/**
	 * Converts a hex color string (e.g. "#1E90FF") to the corresponding Bukkit hex
	 * color code.
	 *
	 * @param hex
	 *            the hex color string (must start with "#")
