...
/**
 * MythicRegistrar is responsible for registering custom mechanics with
 * MythicMobs.
 * 
 * <pre>
...
public class MythicRegistrar {
	...
	private final ContextLogger logger = new ContextLogger(ContextLogger.ContextType.SUB_SYSTEM, "MYTHIC-REGISTRAR");
	...
	/**
	 * Constructs a new MythicRegistrar for the given plugin and pre-populates the
	 * mechanic map.
	 *
	 * @param plugin
	 *            the Plugin instance used for registering mechanics and logging
	 */
	...
	/**
	 * Returns the singleton instance. Must be initialized first with
	 * `initialize(plugin)`.
	 *
	...
	/**
	 * Initializes the singleton instance. This should be called only once during
	 * plugin enable phase.
	 *
	 * @param plugin
	 *            Plugin instance
	...
	/**
	 * Adds a new mechanic to the registrar, attaching the <em>VUTL_</em> identifier
	 * to the front of it for internal purposes.
	 *
	 * @param mechanicName
	 *            the key/name for the mechanic.
	 * @param mechanic
	 *            the Class object for the mechanic implementing BaseMechanic. This
	 *            mechanic must have a constructor like this having higher priority:
	 * 
	 *            <pre> {@code
	...
	public void addMechanic(String mechanicName, Class<? extends BaseMechanic> mechanic) {
		BiFunction<MythicLineConfig, Plugin, BaseMechanic> factory = (config, plugin) -> {
			try {
				return mechanic.getConstructor(MythicLineConfig.class, Plugin.class).newInstance(config, plugin);
			} catch (Exception e) {
				try {
					return mechanic.getConstructor(MythicLineConfig.class).newInstance(config);
				} catch (Exception ex) {
					throw new InvalidMechanicConstructorException("While adding mechanic", mechanicName, ex);
				}
			}
		};
	 ...
	 *
	 * @param mechanicName
	 *            the key/name for the mechanic (without VUTL_ prefix)
	 * @param factory
	 *            the lambda that creates a BaseMechanic
	 */
	public void addMechanic(String mechanicName, BiFunction<MythicLineConfig, Plugin, BaseMechanic> factory) {
		registerMechanicNames(mechanicName, factory);
	}
	 ...
	 *
	 * @param conditionName
	 *            the key/name for the condition (without VUTL_ prefix)
	 * @param factory
	 *            the lambda that creates a BaseMechanic
	 */
	public void addCondition(String conditionName, BiFunction<MythicLineConfig, Plugin, BaseCondition> factory) {
		String base = conditionName.toUpperCase();
		conditionMap.put(base, factory);
	}

	private void registerMechanicNames(String name, BiFunction<MythicLineConfig, Plugin, BaseMechanic> factory) {
	...
	/**
	 * Inner class that listens for MythicMechanicLoadEvent events and registers the
	...
	private class RegistrarListener implements Listener {
		 ...
		 * </p>
		 * <p>
		 * <em> Looks up the mechanic name (converted to uppercase) in the mechanicMap.
		 * If a corresponding factory is found, it creates a new instance of the
		 * mechanic using the event’s configuration and registers it. Otherwise, logs a
		 * warning. </em>
		 * </p>
		 *
		 * @param event
		...
		public void onMythicMechanicLoad(MythicMechanicLoadEvent event) {
			...
			if (upper.startsWith("VUTLS:")) {
				BiFunction<MythicLineConfig, Plugin, BaseMechanic> factory = mechanicMap.getOrDefault(upper,
						mechanicMap.get(upper.replace("VUTLS:", "")));
				if (factory != null) {
					BaseMechanic mechanic = factory.apply(event.getConfig(), plugin);
					event.register(mechanic);
					stringStorer.storeString(plugin.getName(), "mm_mechanics", "-- Registered " + upper + " mechanic!");
				} else {
					logger.print("-- Unrecognized mechanic: " + name, ContextLogger.LogType.WARNING);
				}
			}
		}
		...
		public void onMythicConditionLoad(MythicConditionLoadEvent event) {
			...
			BiFunction<MythicLineConfig, Plugin, BaseCondition> factory = conditionMap.getOrDefault(upper,
					conditionMap.get(prefixed));

			if (factory != null) {
				BaseCondition condition = factory.apply(event.getConfig(), plugin);
				event.register(condition);
				stringStorer.storeString(plugin.getName(), "mm_conditions", "-- Registered " + upper + " condition!");
			} else if (upper.startsWith("VUTL_")) {
				logger.print("-- Unrecognized condition: " + name, ContextLogger.LogType.WARNING);
			}
		}
	}
}
