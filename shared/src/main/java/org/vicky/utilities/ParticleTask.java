...
public class ParticleTask implements Runnable {
  ...
  public ParticleTask(
      ...
      double heightStep,
      IColor headColor,
      IColor transitionColorStart,
      IColor transitionColorEnd,
      int headCount,
      int middleCount,
      double spreadXH,
      double spreadYH,
  ...
  public void alignParticlesToArrow(PlatformLocation[] positions, Arrow arrow) {
      ...
      for (int i = 0; i < positions.length; i++) {
          Vec3 particleVector = positions[i].toVector().subtract(arrow.getLocation().toVector());
          Vec3 rotatedVector = rotateVectorByQuaternion(particleVector, rotation);
  ...
  public Quaternion getRotationTo(Vec3 from, Vec3 to) {
      Vec3 axis = from.clone().crossProduct(to).normalize();
  ...
  public Vec3 rotateVectorByQuaternion(Vec3 v, Quaternion q) {
   ...
   * @return A new PlatformLocation that is the rotated position of the particle relative to the arrow.
   ...
   private PlatformLocation rotateAroundArrow(PlatformLocation particleLoc, PlatformLocation arrowLoc, Vec3 direction) {
    ...
    Vec3 rotatedPos = new Vec3(xPrime, yPrime, zPrime);
