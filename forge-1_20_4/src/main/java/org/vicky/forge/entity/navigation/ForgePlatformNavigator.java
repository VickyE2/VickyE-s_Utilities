...
public class ForgePlatformNavigator implements PathNavigator {
	...
	public static ForgePlatformNavigator from(PathNavigation nav) {
		return new ForgePlatformNavigator(nav);
	}
	...
	public @Nullable AbstractPath createPath(@NotNull Set<? extends IntVec3> set, int i) {
		var BlockPosMap = set.stream().map(ForgeHacks::fromVicky).collect(Collectors.toSet());
