...
public class JsonConfigManager {
	...
	public void createConfig(String path) {
		...
		loader = JacksonConfigurationLoader.builder().path(configFile.toPath()).indent(2)
				.fieldValueSeparatorStyle(FieldValueSeparatorStyle.SPACE_BOTH_SIDES).build();
		...
		if (!configFile.exists()) {
			try {
				...
				logger.print("Failed to create new json file: " + e.getMessage(), ContextLogger.LogType.ERROR);
			}
		} else {
			loadConfigValues();
		}
	...
	public synchronized void saveConfig() {
		try {
			...
			logger.print("Could not save config asynchronously: " + e.getMessage(), ContextLogger.LogType.ERROR);
			if (e.getCause() instanceof AccessDeniedException) {
				logger.print("Access denied while saving config. Check file permissions.", ContextLogger.LogType.ERROR);
			}
		}
	}
	...
	public CompletableFuture<Void> createConfigAsync(String path) {
		...
		}).thenCompose(v -> loadConfigValuesAsync()); // Chain loading of the config after file creation
	}
	...
	public CompletableFuture<Void> loadConfigValuesAsync() {
		return CompletableFuture.runAsync(() -> {
			try {
				...
				logger.print("Failed to load configurations: " + e, ContextLogger.LogType.WARNING);
			}
		});
	}
	...
	public CompletableFuture<Void> saveConfigAsync() {
		return CompletableFuture.runAsync(() -> {
			try {
				...
				logger.print("Could not save config asynchronously: " + e.getMessage(), ContextLogger.LogType.ERROR);
				if (e.getCause() instanceof AccessDeniedException) {
					logger.print("Access denied while saving config. Check file permissions.",
	...
	public CompletableFuture<Boolean> getBooleanValueAsync(String path) {
		return CompletableFuture.supplyAsync(() -> rootNode.node((Object[]) path.split("\\.")).getBoolean());
	}

	// Get double config value asynchronously
	public CompletableFuture<Double> getDoubleValueAsync(String path) {
		return CompletableFuture.supplyAsync(() -> rootNode.node((Object[]) path.split("\\.")).getDouble());
	}

	// Get integer config value asynchronously
	public CompletableFuture<Integer> getIntegerValueAsync(String path) {
		return CompletableFuture.supplyAsync(() -> rootNode.node((Object[]) path.split("\\.")).getInt());
	}

	// Get string config value asynchronously
	public CompletableFuture<String> getStringValueAsync(String path) {
		return CompletableFuture.supplyAsync(() -> rootNode.node((Object[]) path.split("\\.")).getString());
	}
}
