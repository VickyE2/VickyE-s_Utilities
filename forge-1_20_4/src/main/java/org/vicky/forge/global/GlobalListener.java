...
public class GlobalListener {
    ...
    static final MusicPlayerDAO mDao = MusicPlayerDAO.INSTANCE;
    ...
    public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        TransactionCreator.transaction(em -> {
    ...
    public static void onRegisterCommands(RegisterCommandsEvent event) {
        ...
        dispatcher.register(Commands.literal("vicky_utils").then(Commands.literal("open_library").executes(context -> {
            if (context.getSource().getEntity() instanceof ServerPlayer player) {
                var musicPlayer = mDao.findById(player.getUUID());
                ...
                var packet = new OpenOwnedRecordsScreen(
                        musicPlayer.get().getOwnedPieces().stream().map(piece -> piece.getMusicPiece().getId()).toList());
