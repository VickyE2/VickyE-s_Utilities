...
public class MusicRegistry extends Registry<MusicPiece, MusicRegistry> {
	...
	static {
		genreColors.putAll(Map.of("ROCK", NamedTextColor.BLACK, "BLUES", TextColor.color(0, 200, 255), "CHILL",
				TextColor.color(88, 113, 255), "BATTLE", TextColor.color(255, 44, 44), "RETRO",
				TextColor.color(255, 150, 44), "DEFAULT", TextColor.color(180, 180, 180)));
	}
	...
	public MusicRegistry() {
		...
		for (var genreColor : genreColors.entrySet()) {
			if (manager.getConfigValue(genreColor.getKey()) != null) {
				genreColors.put(genreColor.getKey(), TextColor.color(manager.getIntegerValue(genreColor.getKey())));
			}
	...
	public void register(MusicPiece child) {
		getLogger().pending("Adding music: {}", child.pieceName());
		musicPieces.add(child);
		var couldBe = new MusicPieceDAO().findById(child.key());
		if (couldBe.isEmpty()) {
			var piece = new org.vicky.utilities.DatabaseManager.templates.MusicPiece(child.key(), child.pieceName(),
					child.genre(), child.authors());
	...
	public void playPiece(String key, PlatformPlayer toListen) {
		if (musicPieces.stream().noneMatch(p -> p.key().equals(key))) {
			...
			getLogger().warn("Music Piece with key '{}' not found on registry", key);
			return;
		}
		MusicPiece piece = musicPieces.stream().filter(p -> p.key().equals(key)).findFirst().get();
		toListen.sendMessage(Component.text("♫ Now Playing: ", NamedTextColor.GOLD, TextDecoration.BOLD)
				.append(Component.text(piece.pieceName(), TextColor.fromHexString("#b25bb4"), TextDecoration.ITALIC))
	...
	public Component renderMusicPage(PlatformPlayer player, int page) {
		...
		final var dbMusic = MusicPlayerDAO.INSTANCE.findById(player.uniqueId()).get();
		...
		for (var key : dbMusic.getOwnedPieces()) {
			fullOwnedList
					.add(musicPieces.stream().filter(k -> k.key().equals(key.getMusicPiece().getId())).findAny().get());
		}

		List<MusicPiece> pageItems = fullOwnedList.subList(start, end);
		Component header = Component.text("🎵 Music Library (Page " + page + "/" + totalPages + ")",
		...
		for (MusicPiece piece : pageItems) {
			index++;
			list = list.appendNewline()
					.append(Component.text(index + "> ♫ " + piece.pieceName(), NamedTextColor.LIGHT_PURPLE)
							.hoverEvent(HoverEvent.showText(Component.text("Click to play!", NamedTextColor.GREEN)))
							.clickEvent(ClickEvent.runCommand("/play_piece " + piece.key())))
					.append(Component.text(" - More Information", NamedTextColor.GOLD, TextDecoration.BOLD)
							.hoverEvent(HoverEvent.showText(Component.text(Arrays.toString(piece.authors()))
									.appendNewline().append(Component.text("Genre: " + piece.genre()).color(genreColors
											.getOrDefault(piece.genre().toUpperCase(), NamedTextColor.GREEN))))));
		}

		Component footer = Component.empty();
		if (page > 1) {
			footer = footer.append(Component.text("« Prev", NamedTextColor.AQUA)
		...
		if (page < totalPages) {
			footer = footer.append(Component.text("Next »", NamedTextColor.AQUA)
					.clickEvent(ClickEvent.runCommand("/music_piece page " + (page + 1)))
