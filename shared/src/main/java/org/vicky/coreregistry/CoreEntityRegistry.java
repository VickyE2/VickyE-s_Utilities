...
/**
 * Core-side registry for MobEntityDescriptors.
...
public class CoreEntityRegistry {
    ...
    public static void registerAll(Collection<MobEntityDescriptor> descriptors) {
        ...
        for (MobEntityDescriptor d : descriptorsStatic) register(d);
    }
    ...
    public static void installInto(PlatformPlugin plugin) {
        ...
        for (MobEntityDescriptor d : getRegisteredDescriptors()) {
            try {
                ...
                plugin.getPlatformLogger().error("Failed to register core entity descriptor: " + d.getMobDetails().getMobKey(), ex);
            }
        }
    }
    ...
    public static void registerAndCreateNow(MobEntityDescriptor descriptor, PlatformPlugin plugin) {
        ...
        if (plugin != null) {
            try {
                ...
                plugin.getPlatformLogger().error("registerAndCreateNow failed for " + descriptor.getMobDetails().getMobKey(), ex);
            }
        }
    }
}
