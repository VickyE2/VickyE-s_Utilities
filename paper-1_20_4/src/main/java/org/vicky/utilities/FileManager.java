...
public class FileManager {
	...
	private final ContextLogger logger = new ContextLogger(ContextLogger.ContextType.SYSTEM, "EXTRACTOR");
	...
	public void extractDefaultIAAssets(List<String> folders) {
		try {
			...
			try (ZipInputStream zip = new ZipInputStream(jarUrl.openStream())) {
				logger.print("Extracting assets...", ContextLogger.LogType.PENDING);
				...
				logger.print("Extraction complete!", ContextLogger.LogType.SUCCESS);
			}
		} catch (IOException e) {
			logger.print("ERROR EXTRACTING assets! StackTrace:", true);
			e.printStackTrace();
		}
	}

	/**
	 * Extracts a file from the JAR into the destination directory. It ensures the
	 * entire directory structure is created.
	 *
	 * @param destRoot
	 *            The destination root folder (e.g., ItemsAdder directory).
	 * @param entryName
	 *            The name of the file in the JAR to extract.
	 * @throws IOException
		...
		if (!destFile.exists()) {
			try (InputStream in = plugin.getResource(entryName);
				...
				if (in != null) {
					...
					logger.print("Extracted: " + entryName, ContextLogger.LogType.BASIC);
				} else {
					logger.print("Resource not found: " + entryName, ContextLogger.LogType.WARNING);
				}
			}
		}
	}
