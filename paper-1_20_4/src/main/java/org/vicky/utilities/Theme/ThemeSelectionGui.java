...
public class ThemeSelectionGui extends BaseGui {
	/**
	 * Constructs a ThemeSelectionGui with a specified plugin and a custom GUI
	 * listener.
	 *
	 * @param plugin
	...
	public void showGui(Player player) {
		...
		themes.forEach(theme -> {
			GuiCreator.ItemConfig config = GuiCreator.ItemConfigFactory
					.fromComponents(null, SmallCapsConverter.toSmallCaps(theme.getName()), "", true, null,
							"vicky_themes:icon_" + theme.getId(), List.of(Component.text(theme.getDescription(),
									TextColor.color(255, 215, 0), TextDecoration.ITALIC)),
							ButtonAction.ofRunCode(event -> {
								DatabasePlayerDAO dao = new DatabasePlayerDAO();
								Optional<DatabasePlayer> playerOptional = dao.findById(player.getUniqueId());
			...
			guiManager.paginated(player, 6, GuiCreator.ArrowGap.MEDIUM, new ArrayList<>(), themeConfigs, "", 0, 0, "",
					true, true, "", -8);
		});
	}
}
