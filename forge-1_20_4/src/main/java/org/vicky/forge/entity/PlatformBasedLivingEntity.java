...
import org.vicky.forge.forgeplatform.useables.animations.ExtendedAnimationController;
...
public class PlatformBasedLivingEntity extends PathfinderMob implements GeoEntity {
    ...
    public void registerControllers(
        ...
        getDescriptor()
                ...
                .forEach(layer -> controllers.add(
                        new ExtendedAnimationController<>(
                                this,
                                layer.getName(),
                                5,
                                state -> {

                                    EntityAnimationContext context =
                                            createContext(state.getDelegate());

                                    return state.setAndContinue(
                                            layer.getResolver().invoke(context)
                                    );
                                }
                        )
                ));
    ...
    public void forcePause() {
        if (this.actionController == null) return;
        this.actionController.pauseAnimation();
    }

    public void forceResume() {
        if (this.actionController == null) return;
        this.actionController.resumeAnimation();
    }
