...
public class ForgePlatformEntity implements PlatformEntity {
	...
	protected ForgePlatformEntity(Entity ordinal) {
		...
		this.animationController = ForgePlatformAnimationController.from(ordinal);
	}
	...
	public @NotNull PlatformLocation getLocation() {
		return ForgePlatformLocationAdapter
				.fromNativeS(new ForgeVec3(ordinal.level(), ordinal.blockPosition(), ordinal.xRotO, ordinal.yRotO));
	}
	...
	public void teleport(@NotNull PlatformLocation platformLocation) {
		if (platformLocation instanceof ForgeVec3 i)
			if (i.getForgeWorld() instanceof ServerLevel l)
				ordinal.teleportTo(l, i.x, i.y, i.z, Set.of(), ordinal.yRotO, ordinal.xRotO);
	}
	...
	public @Nullable Vec3 getVelocity() {
		return Vec3.of(ordinal.getDeltaMovement().x, ordinal.getDeltaMovement().y, ordinal.getDeltaMovement().z);
	}
	...
	public @NotNull Vec3 getEyeLocation() {
		return Vec3.of(ordinal.getEyePosition().x, ordinal.getEyePosition().y, ordinal.getEyePosition().z);
	}
	...
	public @NotNull AABB getBoundingBox() {
		return new AABB(ordinal.getBoundingBox().minX, ordinal.getBoundingBox().minY, ordinal.getBoundingBox().minZ,
				ordinal.getBoundingBox().maxX, ordinal.getBoundingBox().maxY, ordinal.getBoundingBox().maxZ);
	}

	@Override
	public void setBoundingBox(@NotNull AABB aabb) {
		ordinal.setBoundingBox(net.minecraft.world.phys.AABB.ofSize(
				new net.minecraft.world.phys.Vec3(aabb.getCenter().x, aabb.getCenter().y, aabb.getCenter().z),
				aabb.getWidth(), aabb.getHeight(), aabb.getDepth()));
	}
