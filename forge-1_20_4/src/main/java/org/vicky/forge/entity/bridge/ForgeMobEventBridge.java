 ...
 *
 * This class must be registered on the Forge EVENT BUS.
 * It translates Forge events into calls on MobEntityEventHandler.
 */
...
public final class ForgeMobEventBridge {
    ...
    public static void onLivingAttacked(LivingAttackEvent event) {
        if (!(event.getEntity() instanceof PlatformBasedLivingEntity victim)) return;
        if (!(event.getSource().getEntity() instanceof LivingEntity attacker)) return;

        var handler = victim.getDescriptor().getEventHandler();

        EventResult result = handler.getHandler().onAttacked(
                victim.asPlatform(),
    ...
    public static void onLivingDamage(LivingDamageEvent event) {
        if (!(event.getSource().getEntity() instanceof PlatformBasedLivingEntity attacker)) return;

        var handler = attacker.getDescriptor().getEventHandler();

        EventResult result = handler.getHandler().onAttack(
                attacker.asPlatform(),
                ForgePlatformLivingEntity.from(attacker)
        );
    ...
    public static void onPotionApplied(MobEffectEvent.Added event) {
        if (!(event.getEntity() instanceof PlatformBasedLivingEntity entity)) return;

        var handler = entity.getDescriptor().getEventHandler();

        EventResult result = handler.getHandler().onApplyPotion(
                entity.asPlatform(),
                PlatformPlugin.effectBridge().getEffect(toVicky(event.getEffectInstance().getEffect()
