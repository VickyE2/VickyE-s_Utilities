...
public class MusicBuilder {
	 ...
	 *
	 * <ul>
	 * <li>"C^2" or "C>2" – volume doubled
	 * <li>"D∨2" or "D<2" – volume halved
	 * <li>"[C,.,D,.,E]^3" – volume tripled for block
	 * </ul>
	 *
	 * If no modifier is found, volumeMultiplier is 1.0. Supported symbols:
	 *
	 * <ul>
	 * <li><code>^</code> → increase
	 ...
	 * volume modifier (e.g., "D^2" or "E∨2").
	 * <li>Rests represented by ".".
	 * <li>Smooth progressions (e.g., "E-8>E+"). The duration is absolute.
	 * <li>Chords: notes separated by "-" (e.g., "E—D" for a duo, "E—D—F" for a
	 * triad, etc.).
	 * <li>Tempo modifier blocks: tokens in the form "[...]" optionally followed by
	 * a tempo multiplier. For example, "[D,.,.,.,F,.,.,.,G]^2" applies a volume
	 * modifier to every note in the block and also adjusts timing.
	 * <li>Streached Notes (e.g., "C->n"). n is the number of seconds the note is
	 * stretched for. also works for chords C—E—G->n
	 * </ul>
	 *
	 * The totalTime (in ticks) is divided among non-absolute events (unless
	 * overridden by absolute durations or tempo modifiers). The base volume (passed
	 * as a parameter) is modified on a per-token basis.
	 *
	 * <p>
	 * e.g.
	 *
	 ...
	 * @param totalTime
	 *            the total duration (in ticks) allocated for non-absolute events.
	 * @param baseVolume
	...
	private void parseAndSchedule(List<String> tokens, int start, int end, long interval, float baseVolume, Sound sound,
	...
	private long handleBlock(String token, long interval, float baseVolume, Sound sound, MusicBuilder builder,
		...
		parseAndSchedule(innerTokens, 0, innerTokens.size(), innerInterval, baseVolume, sound, builder, currentTime);
	...
	private static long handleChord(String token, Sound sound, float volume, MusicBuilder builder, long currentTime,
			float tempo, long interval) {
		String[] notes = token.split("—");
		List<Float> pitches = Arrays.stream(notes).map(MusicBuilder::pitchFor).toList();
		switch (pitches.size()) {
			case 2 -> builder.addDuo(currentTime, sound, pitches.get(0), pitches.get(1), volume, SoundCategory.MUSIC);
			...
			case 4 -> builder.addQuad(currentTime, sound, pitches.get(0), pitches.get(1), pitches.get(2),
					pitches.get(3), volume, SoundCategory.MUSIC);
	 ...
	 *
	 * <ul>
	 * <li>C – base pitch 0.5f
	 * <li>D – base pitch 0.6f
	 * <li>E – base pitch 0.7f
	 * <li>F – base pitch 0.8f
	 * <li>G – base pitch 0.9f
	 * <li>A – base pitch 1.0f
	 * <li>B – base pitch 1.1f
	 * <li>C+ – base pitch 1.2f (octave up)
	 * </ul>
	 *
	 ...
	 */
	public MusicBuilder addSingle(long timeOffset, Sound sound, float pitch, float volume, SoundCategory category) {
		track.addEvent(new MusicEvent(timeOffset, sound, pitch, volume, category));
		return this;
	}

	public MusicBuilder addSustainedNote(long startTime, Sound sound, float pitch, float volume, long durationTicks,
	...
	public MusicBuilder addSustainedChord(long startTime, Sound sound, List<Float> pitches, float volume, long duration,
			SoundCategory category) {
	...
	private void addSmoothProgression(long startTime, Sound sound, float startPitch, float endPitch, int duration,
			SoundCategory category) {
		// For example, you might break the duration into smaller steps and call
		// addSingle for each.
	 ...
	 * @return this builder for chaining.
	 */
	public MusicBuilder smoothProgress(double startPitch, double endPitch, long duration, Sound sound, float volume,
	 ...
	 * @return this builder for chaining.
	 */
	public MusicBuilder progress(double startPitch, double endPitch, long duration, Sound sound, float volume,
	 ...
	 * @return this builder for chaining.
	 */
	public MusicBuilder addDuo(long timeOffset, Sound sound, float pitch1, float pitch2, float volume,
			SoundCategory category) {
	 ...
	 * @return this builder for chaining.
	 */
	public MusicBuilder addTriple(long timeOffset, Sound sound, float pitch1, float pitch2, float pitch3, float volume,
			SoundCategory category) {
		addSingle(timeOffset, sound, pitch1, volume, category);
		addSingle(timeOffset, sound, pitch2, volume, category);
	 ...
	 * @return this builder for chaining.
	 */
	public MusicBuilder addQuad(long timeOffset, Sound sound, float pitch1, float pitch2, float pitch3, float pitch4,
