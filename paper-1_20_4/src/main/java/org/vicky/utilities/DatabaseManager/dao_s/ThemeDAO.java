...
import org.vicky.shaded.jakarta.persistence.EntityTransaction;
...
public class ThemeDAO {
    ...
    public Theme findById(String id) {
        try (EntityManager em = HibernateUtil.getEntityManager()) { // Open new EntityManager
            Theme theme = em.createQuery("FROM Theme WHERE id = :id", Theme.class).setParameter("id", id)
                    .getSingleResult();
    ...
    public Theme findByName(String name) {
        try (EntityManager em = HibernateUtil.getEntityManager()) {
            ...
            Theme theme = em.createQuery("FROM Theme WHERE name = :name", Theme.class).setParameter("name", name)
                    .getSingleResult();
            return theme;
        } catch (NoResultException e) {
            return null;
