...
public interface PlatformWorld<T, N> {
	...
	@NotNull
	List<? extends @NotNull PlatformEntity> getEntities();

	@NotNull
	List<? extends @NotNull PlatformEntity> getEntitiesWithin(double x, double y, double z, float range);

	@NotNull
	List<? extends @NotNull PlatformLivingEntity> getLivingEntitiesWithin(double x, double y, double z, float range);
	 ...
	 * @param state
	 *            The new block state to set.
	 */
	void setPlatformBlockState(Vec3 position, PlatformBlockState<T> state);

	/**
	 * Similar to
	 * {@link PlatformWorld#setPlatformBlockState(Vec3, PlatformBlockState)} but
	 * allows for setting nbt mainly for block entities
	 *
	 * @param position
	 *            The location to modify.
	 * @param state
	 *            The new block state to set.
	 * @param nbt
	 *            The compound nbt to parse. Each Impl would do this their own way
	 */
	...
	/**
	 * Ensures the chunk containing this block position is loaded. Platform-specific
	 * implementations handle the actual chunk logic.
	 */
