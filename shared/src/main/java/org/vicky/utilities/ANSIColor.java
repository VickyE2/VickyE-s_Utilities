...
public class ANSIColor {
	 ...
	 *
	 * <ul>
	 * <li><b>Basic Colors:</b> black, red, green, yellow, blur, purple, cyan, white
	 * <li><b>Bold Colors:</b> add _bold to the end of the basic colors
	 * <li><b>Light Colors:</b> add bright_ to the start of the basic colors except
	 * black and white which become bright_grey
	 * <li><b>Dark Colors:</b> add dark_ to the start of the basic colors excluding
	 * white and black which become dark_ grey
	 * <li><b>Styles:</b> bold, bold_italic, italic, strikethrough, underline
	 * </ul>
	 *
	 * <br>
	 *
	 * @param message
	 *            This is the string the ansi color is to be applied to
	 * @param color
	 *            The specific ansi color to apply
	 * @return Returns an ansi enriched text
	 * @deprecated This method is rather too rigid and dosent support multi colors
	 *             on a single string without splitting it. I might remove this
	 *             method later.....might...
	 *             <p>
	 *             Use {@link #colorize(String)}
	 */
	@Deprecated
	 ...
	 * <em><b>{@code ('color[style[...message...]] color[...message...]
	 * ...')}</b></em>
	 *
	 * <p>
	 * styles should come inside the color.
	 *
	 * @param message
	 *            This is the string containing all the regex-color-message
	 * @return Returns an ansi enriched text
	 */
	public static String colorize(String message) {
		...
		while (true) {
			...
			String colorCode = color.contains("#") ? ansiFromHex(color) : COLOR_MAP.getOrDefault(color, RESET);
	...
	/**
	 * Applies a rainbow effect to the text inside a marker. If a segment length is
	 * provided (e.g., rainbow-5-red-blue[...]), then the text is divided into
	 * segments of that length and each segment is colored. Otherwise, the text is
	 * split by whitespace, and each word is colored.
	 *
	 * @param message
	 *            the input string in rainbow format
	...
	public static String rainbowColorize(String message) {
		...
		for (String name : colorNames) {
			String code = name.contains("#") ? ansiFromHex(name) : COLOR_MAP.getOrDefault(name.toLowerCase(), RESET);
			colorCodes.add(code);
		}
	...
	/**
	 * Processes a string that may contain simple color markers, rainbow markers, or
	 * gradient markers, and returns the string with ANSI color codes applied.
	 *
	 * <p>
	 * Supported formats: - Simple color: "red[Hello]" or "#AA0000[Hello]" -
	 * Rainbow: "rainbow-5-red-blue[World]" - Gradient: * Basic:
	 * "gradient-#FF0000-#00FF00[Gradient Text]" (default alignment "left") * With
	 * alignment: "gradient-center-#FF0000-#00FF00[Gradient Text]" * Manual
	 * multi-line: "gradient-45deg-top-#FF0000-#00FF00-#0000FF[Line1\nLine2\nLine3]"
	 *
	 * @param message
	 *            The input string with markers.
	...
	public static String colorizeMixed(String message) {
		...
		while (matcher.find()) {
			...
			// Else, it's a simple color marker (group 5)
				...
				String colorCode = marker.startsWith("#") ? ansiFromHex(marker) : COLOR_MAP.getOrDefault(marker, RESET);
	 ...
	 *
	 * @param input
	 *            The string to be cleaned of ansi codes
	 * @return This returns the inputted string with no ansi codes for log-saving
	 *         functionalities
	 */
	 ...
	 *
	 * <p>
	 * Note: ANSI does not support true hex colors. This method is a placeholder and
	 * may need to map the hex code to one of the supported ANSI colors.
	 *
	 * @param hex
	 ...
	 *
	 * @param startColor
	 *            the start hex color (e.g. "#FF0000")
	 * @param endColor
	 *            the end hex color (e.g. "#0000FF")
	 * @param ratio
	 *            a value between 0.0 (startColor) and 1.0 (endColor)
	 ...
	 *
	 * @param message
	 *            The full gradient marker string.
	 ...
	 * </pre>
	 *
	 * The angle (in degrees) is used to adjust the interpolation non-linearly. The
	 * direction can be:
	 *
	 * <ul>
	 * <li>"top" (vertical gradient; top line uses first color, bottom uses last)
	 * <li>"bottom" (vertical gradient reversed)
	 * <li>"left" (horizontal gradient on each line; leftmost character uses first
	 * color)
	 * <li>"right" (horizontal gradient reversed)
	 * </ul>
	 *
	 * @param marker
	 *            The full gradient marker string.
	 ...
	 *
	 * @param text
	 *            The text line to colorize.
	 * @param direction
	 *            The direction: "left" means gradient from left (first color) to
	 *            right (last color), "right" means reversed.
	 * @param colors
	 *            The list of hex color stops.
	 * @param factor
	 *            The non-linear factor derived from the angle.
	...
	private static String gradientHorizontal(String text, String alignment, String startColor, String endColor) {
		...
		if (textLength == 0)
			return text; // If no visible text, return as-is
	...
	private static String gradientVertical(String text, String alignment, String startColor, String endColor) {
		String[] lines = text.split("\n");
	 ...
	 *
	 * @param colorName
	 *            The name of the new color (e.g., "orange").
	 * @param ansiCode
	 *            The ANSI escape sequence for the color (e.g., "\u001B[38;5;214m").
	 */
