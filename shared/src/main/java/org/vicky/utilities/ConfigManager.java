...
public class ConfigManager {
	...
	private final PlatformScheduler scheduler = PlatformPlugin.server().getScheduler();

	/**
	 * Creates the config in the path specified with the parent dir being the
	 * {@link PlatformPlugin} datafolder {@link PlatformPlugin#dataFolder()}
	 *
	 * @param path
	 *            The path of the config file (.yml)
	 */
	 ...
	 *            The path of the config file (.yml)
	 * @param shouldLog
	 *            Specifies weather the context logger should give out logs on steps
	 *            taken
	 */
	...
	public void createConfig(String path) {
		...
		options = YamlConfigurationLoader.builder().indent(2).nodeStyle(NodeStyle.BLOCK).defaultOptions()
				.serializers(builder -> builder.registerAnnotatedObjects(factory));

		loader = YamlConfigurationLoader.builder().path(configFile.toPath()).indent(2).nodeStyle(NodeStyle.BLOCK)
				.build();
		...
		if (!configFile.exists()) {
			try {
				...
				logger.print("Failed to create new config file: " + e.getMessage(), ContextLogger.LogType.ERROR);
			}
		} else {
			loadConfigValues();
		}
			...
			if (entry != null && !entry.isDirectory()) {
				try (InputStream inputStream = zipFile.getInputStream(entry);
					...
					YamlConfigurationLoader loader = YamlConfigurationLoader.builder()
							.source(() -> new BufferedReader(reader)).nodeStyle(NodeStyle.BLOCK).build();
					...
					// Log success
					if (shouldLog)
						logger.print(configFileName + " Has been successfully loaded from zip-file: "
								+ zipFilePath.getFileName());
				}
			} else {
				if (shouldLog)
					logger.print("Could not find the required '" + configFileName + "' config file inside zip-file: "
							+ zipFilePath.getFileName());
			}
		} catch (Exception e) {
			logger.print("Failed to load configurations from zip file: " + zipFilePath.getFileName() + " reason: "
					+ e.getMessage());
		}
	}
	...
	public void setListConfigValue(String key, List<String> values) {
		try {
			...
			logger.print("Failed to set config value for " + key + ": " + e.getMessage(),
	...
	public List<String> getListConfigValue(String key) {
		...
		try {
			...
			logger.print("Failed to get list config value for " + key + ": " + e.getMessage(),
	...
	public void setConfigValue(String parentKey, String childKey, Object value, String comment) {
		try {
			...
			logger.print("Failed to add config: " + parentKey + "." + childKey + " with value: " + value,
					ContextLogger.LogType.WARNING);
		}
		saveConfig(); // Save changes to the config file
	}
