...
public class CommunicatorServer {
  private static final ContextLogger logger =
      new ContextLogger(ContextLogger.ContextType.SUB_SYSTEM, "COMMUNICATOR");
  ...
  public static void startupCommunicationServer() {
    ...
    logger.printBukkit(
        ANSIColor.colorize(
            "green_bold[✓] Communication Server is now online! Listening for plugin"
                + " registrations..."),
   ...
   *
   * @param name   Plugin name (unique)
   * @param loader Plugin classloader
   * @param impl   Plugin implementation (extends Communicateable)
   * @return PluginCommunicator for the registered plugin
   */
  public synchronized PluginCommunicator register(
      String name, ClassLoader loader, Communicateable impl) {
    ...
    if (previous != null) {
      logger.printBukkit(
          ANSIColor.colorize("yellow[Plugin already registered: " + name + "]. Overwriting..."),
      ...
      try {
        ...
        logger.printBukkit(
            ANSIColor.colorize(
                "red[Error in onShutdown for previous " + name + ": " + e.getMessage() + "]"),
            ContextLogger.LogType.ERROR);
      }
    }

    logger.printBukkit(
        ANSIColor.colorize("green[✓] Plugin hooked: " + name), ContextLogger.LogType.AMBIENCE);
    ...
    try {
      ...
      logger.printBukkit(
          ANSIColor.colorize(
              "red[Error calling onRegister for " + name + ": " + e.getMessage() + "]"),
          ContextLogger.LogType.ERROR);
    }
    return communicator;
  }
  ...
  public CompletableFuture<Object> sendAsync(String targetPlugin, String key, JsonObject message) {
    ...
    if (communicator == null) {
      logger.printBukkit(
          ANSIColor.colorize("red[Target plugin not found: " + targetPlugin + "]"),
          ContextLogger.LogType.ERROR);
  ...
  public synchronized void unregister(String name) {
    ...
    if (communicator != null) {
      try {
        ...
        logger.printBukkit(
            ANSIColor.colorize("red[Error in onShutdown for " + name + ": " + e.getMessage() + "]"),
            ContextLogger.LogType.ERROR);
      }
      logger.printBukkit(
          ANSIColor.colorize("blue[-] Plugin unhooked: " + name), ContextLogger.LogType.BASIC);
    } else {
      logger.printBukkit(
          ANSIColor.colorize("yellow[Plugin not found: " + name + "]"),
          ContextLogger.LogType.WARNING);
    }
  }
  ...
  public synchronized void shutdownAll() {
    ...
    plugins.clear();
    logger.printBukkit("cyan[All plugins unregistered]", ContextLogger.LogType.BASIC);
  }
    ...
    if (plugins.isEmpty()) {
      ...
      for (Map.Entry<String, PluginCommunicator> entry : plugins.entrySet()) {
        ...
        sb.append(ANSIColor.colorize("yellow_bold[") + index++ + ".] ")
            ...
            .append(ANSIColor.colorize("magenta[" + loaderName + "]"))
            .append("\n");
      }
